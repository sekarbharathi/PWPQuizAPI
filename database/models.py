...
class Category(db.Model):
    ...
    quizzes = db.relationship(
        "Quiz", secondary="quiz_category", backref="categories", cascade="all, delete"
    )
...
class QuizCategory(db.Model):
    ...
    quiz_id = db.Column(
        db.Integer, db.ForeignKey("quiz.quiz_id", ondelete="CASCADE"), primary_key=True
    )
...
class Question(db.Model):
    ...
    __table_args__ = (
        sa.CheckConstraint(
            "complex_level IN ('easy', 'medium', 'hard')", name="check_complex_level"
        ),
    )
...
class Quiz(db.Model):
    ...
    questions = db.relationship(
        "Question", secondary="quiz_question", backref="quizzes", cascade="all, delete"
    )
