...
def get_quizzes():
    ...
    quizzes_list = [{"quiz_id": quiz.quiz_id, "name": quiz.name,
...
def create_quiz():
    ...
    return jsonify({"msg": "Quiz created", "quiz_id": new_quiz.quiz_id}), 201
...
def update_quiz(quiz_id):
    ...
    if new_category_id:
        ...
        QuizCategory.query.filter_by(quiz_id=quiz_id).delete()
        new_quiz_category = QuizCategory(
            quiz_id=quiz_id, category_id=new_category_id)
        db.session.add(new_quiz_category)
...
def create_question():
    ...
    return jsonify({"msg": "Question created",
...
def update_question(question_id):
    ...
    if new_quiz_id and new_quiz_id != QuizQuestion.query.filter_by(
            question_id=question_id).first().quiz_id:
        QuizQuestion.query.filter_by(question_id=question_id).delete()
        new_quiz_question = QuizQuestion(
            quiz_id=new_quiz_id, question_id=question_id)
        db.session.add(new_quiz_question)
...
def get_questions():
    ...
    for q in questions:
        ...
        options_list = [{"option_id": opt.option_id,
...
def get_questions_by_quiz(quiz_id):
    ...
    questions = db.session.query(Question).join(
        QuizQuestion).filter(QuizQuestion.quiz_id == quiz_id).all()
    questions_list = []
    for q in questions:
        options = Option.query.filter_by(question_id=q.question_id).all()
        options_list = [{"option_id": opt.option_id,
