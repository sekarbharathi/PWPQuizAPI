...
class QuizResource(MethodView):
    ...
    def get(self):
        ...
        quizzes_list = [
            {"unique_id": quiz.unique_id, "name": quiz.name, "description": quiz.description}
            for quiz in quizzes
        ]
...
class QuestionResource(MethodView):
    ...
    def post(self):
        ...
        new_quiz_question = QuizQuestion(
            quiz_id=quiz_id, question_id=new_question.question_id
        )
        db.session.add(new_quiz_question)
        db.session.commit()

        return jsonify(
            {"msg": "Question created", "unique_id": new_question.unique_id}
        ), 201
...
class QuestionDetailResource(MethodView):
    ...
    def put(self, unique_id):
        ...
        if new_quiz_id:
            new_quiz = Quiz.query.get(new_quiz_id)
            if not new_quiz:
                return jsonify({"msg": "Quiz not found"}), 404
            ...
            if (
                ...
                or current_quiz_question.quiz_id != new_quiz_id
            ):
                QuizQuestion.query.filter_by(
                    question_id=question.question_id
                ).delete()
                new_quiz_question = QuizQuestion(
